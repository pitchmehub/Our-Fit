...
export default function ExploreScreen() {
  ...
  useEffect(() => {
    if (!selectedOutfit) { router.back(); return; }
    ...
    try {
      ...
      await Promise.all(
        concepts.map(async (concept) => {
          try {
            const generated = await generateOutfitImage(concept);
            setDisplayedOutfits((prev) =>
              prev.map((o) => (o.id === generated.id ? generated : o))
            );
          } catch {}
          finally {
  ...
  const handleLike = useCallback(
    (outfit: Outfit) => { if (user?.id) toggleLike(outfit, user.id); },
    [user, toggleLike]
  );
  ...
  if (error) {
    return (
      <View style={[styles.errorContainer, { paddingTop: topPadding }]}>
        ...
        <TouchableOpacity style={styles.retryBtn} onPress={() => currentSelected && loadMore(currentSelected)}>
  ...
  const readyCount = displayedOutfits.filter((o) => o.image !== "").length;
    ...
    <View style={[styles.container, { paddingBottom: bottomPadding }]}>
      <View style={[styles.header, { paddingTop: topPadding }]}>
        ...
        <View style={styles.headerCenter}>
          ...
          {currentSelected && (
            <Text style={styles.selectedTitle} numberOfLines={1}>{currentSelected.title}</Text>
          )}
        </View>
      ...
      {!conceptsLoaded ? (
        <LoadingView message="Explorando mais looks..." subtitle="Buscando referências de estilo" />
      ) : (
        <ScrollView contentContainerStyle={styles.grid} showsVerticalScrollIndicator={false}>
          <Text style={styles.sectionLabel}>
            {loadingImages.size > 0 ? `${readyCount} de 6 carregados` : "Mais looks como este"}
          </Text>
          ...
          {!loadingImages.size && (
            <Text style={styles.tapHint}>Toque em qualquer look para explorar ainda mais</Text>
          )}
          <TouchableOpacity style={styles.newSearchBtn} onPress={() => router.replace("/")} activeOpacity={0.85}>
...
const styles = StyleSheet.create({
  ...
  headerLogo: { color: "#E8FF00", fontSize: 16, fontFamily: "Inter_700Bold", letterSpacing: 5 },
  selectedTitle: { color: "#888888", fontSize: 11, fontFamily: "Inter_400Regular", marginTop: 2, maxWidth: 180 },
  badge: { backgroundColor: "#E8FF00", paddingHorizontal: 10, paddingVertical: 5, borderRadius: 10 },
  badgeText: { color: "#0A0A0A", fontSize: 12, fontFamily: "Inter_700Bold" },
  grid: { paddingHorizontal: 20, paddingTop: 20, paddingBottom: 40 },
  sectionLabel: {
    color: "#888888", fontSize: 12, fontFamily: "Inter_500Medium",
  ...
  newSearchBtn: {
    ...
    justifyContent: "center", gap: 8, paddingVertical: 14, borderRadius: 14, marginTop: 24,
  },
  newSearchText: { color: "#0A0A0A", fontSize: 15, fontFamily: "Inter_600SemiBold", letterSpacing: 0.3 },
  errorContainer: {
