...
export default function ResultsPage() {
  ...
  }, [capturedImage, currentOutfits.length, setLocation]);
    ...
    await Promise.allSettled(
      concepts.map(async (concept) => {
        try {
          ...
          // Optional: handle retry or show error on specific card
        }
      })
    );
  };
  ...
  const handleExploreMore = async () => {
    ...
    try {
      ...
      console.error(err);
      setError("Não foi possível explorar mais looks. Tente novamente.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <header className="sticky top-0 z-20 bg-background/80 backdrop-blur-xl border-b border-border flex items-center p-4 gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground uppercase tracking-wider mt-0.5">
            {loading ? "Analisando sua peça..." : "6 looks gerados"}
          </p>
        </div>
        {capturedImage && (
